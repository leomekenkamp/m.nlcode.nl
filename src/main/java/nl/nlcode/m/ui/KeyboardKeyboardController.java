 ...
 * @author lmekenkamp
 */
public class KeyboardKeyboardController {

    private static final Logger LOGGER = LoggerFactory.getLogger(KeyboardKeyboardController.class);
    ...
    public KeyboardKeyboardController() {
    }
    
    public KeyboardKeyboardController(ProjectUi projectUi, KeyboardKeyboard keyboardKeyboard, MenuItem menuItemt) {
    ...
    public void setParent(KeyboardKeyboardUi parent) {
        KeyboardKeyboard midiInOut = parent.getMidiInOut();
        ...
        velocity.getValueFactory().setValue(parent.getMidiInOut().getVelocity());
        velocity.valueProperty().addListener((ov, oldValue, newValue) -> {
            parent.getMidiInOut().setVelocity(newValue);
            parent.setDirty();
        });
        channel.getValueFactory().setValue(parent.getMidiInOut().getOneBasedChannel());
        channel.valueProperty().addListener((ov, oldValue, newValue) -> {
            parent.getMidiInOut().setOneBasedChannel(newValue);
            parent.setDirty();
        });
        octave.getValueFactory().setValue(parent.getMidiInOut().getOctave());
        octave.valueProperty().addListener((ov, oldValue, newValue) -> {
            parent.getMidiInOut().setOctave(newValue);
            parent.setDirty();
        });
    }
